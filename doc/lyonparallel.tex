...
It implements the notions of DPO rewriting systems on finite directed, edge-labeled multigraphs, the type graph method presented by Endrullis and Overbeek~\cite{endrullis2024generalized_icgt} and its extension~\cite{qiu2025termination_nwf_v2_acceptedgcm}. It also implements the morphism counting method and its extension~\cite{qiu2025termination_icgt}.
For DPO graph rewriting systems with monic matches,
the type graph method presented by Endrullis and Overbeek~\cite{endrullis2024generalized_icgt} is more powerful than the versions presented by Zantema et al. and Bruggink et al.~\cite{zantema2014termination,bruggink2014termination,bruggink2015proving}; these versions are implemented in TORPAcyc~\cite{TORPAcyc} and Grez~\cite{grez}. At the time of writing and to the best of our knowledge, the method presented by Overbeek and Endrullis has no publicly available implementation.

Figure~\ref{fig:lyonparallel_workflow} illustrates the workflow of LyonParallel.
...
\resizebox{\textwidth}{!}{
  ...
  \node[draw, rectangle, below left=8mm of choose] (typegraph) {Type Graph Method};
  \node[draw, rectangle, below right=8mm of choose] (subcount) {Morphism Counting};

  % Z3 step (compact)
  \node[draw, rectangle, below=of typegraph] (z3) {Solve constraints with Z3};
  ...
  \draw[->] (trynext) -- node[midway,left]{No} (end);
    ...
    \caption{Workflow of LyonParallel}
    \label{fig:lyonparallel_workflow}
\end{figure}
...
The representation of a labeled graph is constructed using the function \colorbox{Ivory2}{$MGraph.fromList$} with two parameters:
\begin{enumerate}
    \item a list of integers representing the nodes of the graph, and
    \item a list of four-tuples representing the labeled edges of the graph, where each tuple \colorbox{Ivory2}{$(s,lab,t,id)$} of type \colorbox{Ivory2}{$\textit{int}\times \textit{string}\times \textit{int}\times \textit{int}$} represents an edge with source node $s$, label $lab$, target node $t$, and identifier $id$.
\end{enumerate} 
...
\colorbox{Ivory2}{$GraphHomomorphism.fromList$} with four arguments:
    \begin{enumerate}
...
with the representations of the left- and right-hand side homomorphisms.
For example, 
...
$$\colorbox{Ivory2}{$GraphRewritingSystem.fromRulesListAndName$}$$ with three arguments: a list of rewriting rules and the name of the system; the third parameter is \colorbox{Ivory2}{$false$} by default.
...
A ruler-graph with one forbidden context is represented by a structure with an underlying graph $X$ and an injective graph homomorphism from $X$. 
...
Consider the graph rewriting system with a single rule:
    \begin{center}
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}
        ...
        \end{center}
Its termination cannot be proved using the type graph method presented in~\cite{zantema2014termination,bruggink2014termination,bruggink2015proving,endrullis2024generalized_icgt}. However, its relative termination with respect to the empty set can be proved using the morphism counting method.

The following sequence of commands selects the system, then uses the morphism counting method without forbidden patterns to analyse the relative termination of the system.
\begin{lstlisting}
...
\subsection{Termination analysis using morphism counting with antipatterns}
Consider the graph rewriting rule illustrated below.
\begin{center}
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}
...
The ruler-graph shown in Figure~\ref{fig:implemented:ruler_graph_representation} is predefined and can be selected with the REPL command:
 ...
 \end{lstlisting}
To analyse the system's relative termination using the morphism counting method with a forbidden pattern with the ruler-graph, run:
 \begin{lstlisting}
...
The summary also contains the information shown below.
\begin{center}
    \begin{lstlisting} 
...
This implementation accompanies the thesis available at \url{https://github.com/Qi-tchi/phd_thesis}. The project is distributed under the LGPL-2.1 License.